...
public class Notification {
	...
	public ArrayList<DateTime> getDates() {
		...
		if (mStartYear != -1) {
			dates.add(new DateTime(mStartYear, mStartMonth, mStartDay,
					mStartHours, mStartMinutes));
		} else {
			dates.add(null);
		}
		if (mEndYear != -1) {
			dates.add(new DateTime(mEndYear, mEndMonth, mEndDay, mEndHours,
					mEndMinutes));
		} else {
			dates.add(null);
		}
	...
	public void registerAlarm(Context _context) {
		...
		for (int i = 0; i < dates.size(); i++) {
			if (dates.get(i) != null) {
				// the data gets set here so that the intents become different
				// and will not overwrite each other
				intent.setData(Uri.parse(dates.get(i).toString()));
				PendingIntent pendingIntent = PendingIntent.getBroadcast(
						_context, 0, intent, 0);
				alarmManager.set(AlarmManager.RTC_WAKEUP, dates.get(i)
						.getMillis(), pendingIntent);
				Log.i(TAG, "set notification alarm at: "
	...
	/**
	 * Generates a unique id to use with a notification. The first value is a
	 * random number between 0 and 99, then the current time in ms is
	 * concatenated.
	 * 
	...
	public static long generateUniqueID() {
		...
		long id = Long.parseLong(Byte.toString(first)
